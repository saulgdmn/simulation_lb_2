...
to setup-world
  ...
  ask patches [  ;; set colors for the different sections of the world
    if pycor > sky-top [  ;; space
      set pcolor scale-color white pycor 20 13
    ]
...
to run-sunshine
  ...
  if day? [create-sunshine]  ;; start new sun rays from top
...
to create-sunshine
  ...
  if 10 * sun-brightness > random 50 [
    create-rays 1 [
      let phase-of-day ticks mod day-length / 6
      let angle 110 + (phase-of-day)
      set heading angle
      set color yellow
